...
public class EntityEnemy : MonoBehaviour
{
	...
	void Update()
	{
		if (Time.time - _lastFireTime >= FireInterval)
		{
			_lastFireTime = Time.time;
			_audioSource.Play();
			AppEvent.Send(ActorEventDefine.EnemyFireBullet,this._shotSpawn.position, this._shotSpawn.rotation);
		}
	}
	void FixedUpdate()
	{
	...
	void OnTriggerEnter(Collider other)
	{
		...
		{
			AppEvent.Send(ActorEventDefine.EnemyDead,this.transform.position, this.transform.rotation);
